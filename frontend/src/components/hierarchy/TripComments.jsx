...
export default function TripComments({ trip, onClose }) {
  ...
  const loadComments = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/v2/trips/${trip.id}/comments`);
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await api.post(`/v2/trips/${trip.id}/comments`, { text: newText.trim() });
      setNewText('');
      await loadComments();
    } finally {
      setSaving(false);
  ...
  const handleEdit = async (commentId) => {
    ...
    setSaving(true);
    try {
      await api.put(`/v2/trip-comments/${commentId}`, { text: editText.trim() });
      setEditingId(null);
      await loadComments();
    } finally {
      setSaving(false);
  ...
  const loadHistory = async (commentId) => {
    ...
    try {
      const res = await api.get(`/v2/trip-comments/${commentId}/history`);
      setHistory(res.data || []);
    } catch {
      setHistory([]);
    } finally {
  ...
  const tripLabel = trip.departure_station_name || trip.departure_station_code || '—';
  const destLabel = trip.destination_station_name || trip.destination_station_code || '—';
  const dateLabel = trip.flight_start_date
    ? new Date(trip.flight_start_date).toLocaleDateString('ru-RU')
    : '—';
  ...
  return (
    <div className="modal-overlay" role="dialog" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-content h-comments-modal">
        <h3>Комментарии к рейсу</h3>
        <div className="h-trip-label">{dateLabel} · {tripLabel} → {destLabel}</div>
        ...
        <textarea
          ...
          placeholder="Новый комментарий к рейсу…"
          rows={3}
        />

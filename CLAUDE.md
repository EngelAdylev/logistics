...
- [x] Несвязанные накладные — таблица на странице Поездов (ниже основной таблицы), показывает накладные из etran_waybills, не привязанные к вагонам
- [x] Флаг is_relevant для завершённых накладных (18 терминальных статусов ЭТРАН)
...
- **Группировка (Group by):**
  - Dropdown в toolbar для выбора колонки группировки
  - Автоматическое развёртывание всех групп при выборе группировки
  - Expandable/collapsible группы с иконками ChevronDown/ChevronRight и подсчётом вагонов
  - Пустые значения показываются как "—" в отдельной группе в конце
  - Группировка работает с фильтрами и другими режимами таблицы
...
**Несвязанные накладные (Unbound Waybills — страница уровня):**
- Отдельная таблица НИЖЕ основной таблицы поездов на WagonsPage (вкладка Поезда)
- Разделена горизонтальной линией (1px) от таблицы поездов
- Колонки: Номер накладной (15%), Отправитель (20%), Получатель (20%), Груз (15%), Статус (20%), Тип (10%)
- Показывает все накладные где is_relevant=true и нет связанных TripWaybill
- Обновляется при загрузке страницы и при синхронизации
- Накладная исчезает когда вагон с этой накладной приходит в дислокацию (создаётся TripWaybill через scheduler)
...
## Жизненный цикл накладных ЭТРАН (Waybills Lifecycle)

**Флаг is_relevant:**
- Создание: `is_relevant = not _is_terminal_status(status)`
- Обновление: если статус становится терминальным → `is_relevant = false`
- Использование: несвязанные накладные показываются только если `is_relevant = true`

**18 Терминальных статусов** (после которых накладная скрывается из unbound):
```
груз прибыл, груз завезен полностью, груз не прибыл, 
работа с документом окончена, работа с документами окончена,
испорчен, испорчен после сторно, испорчен с.333 не прошло,
сторнирован в пути, сторнирован по назначению, сторнирован по отправлению,
раскредитован, отклонен, отказ грузоотправителя, отказ от уведомления,
приемосдатчиком принято, грузоотправитель накладную получил, получатель уведомлен
```

**Основной workflow:**
1. Накладная приходит из ЭТРАН (POST /etran/import-waybills)
2. Если статус не терминальный → `is_relevant = true` → видна в unbound
3. Когда вагон приходит в дислокацию с номером накладной → создаётся TripWaybill (связь)
4. Накладная исчезает из unbound (есть в trip_waybills → NOT IN исключит)
5. Если позже статус становится терминальным → `is_relevant = false` (double check)
...
### GET /v2/unbound-waybills
Все несвязанные накладные в системе (глобальный вид).
```
Response: { items: [...], total: int }
...
```
**Условия отбора:** WHERE is_relevant=true AND id NOT IN (SELECT DISTINCT waybill_id FROM trip_waybills)
**Назначение:** Глобальный список накладных, которые ещё не привязаны ни к одному вагону.
Используется в таблице "Несвязанные накладные" на странице Поездов (WagonsPage, вкладка "Поезда").

### GET /v2/routes/{route_id}/unbound-waybills
Несвязанные накладные, специфичные для конкретного маршрута.
```
Response: { items: [...], total: int }
Поля item: id (UUID), waybill_number, shipper_name, consignee_name,
  departure_station_name, destination_station_name, status, waybill_type,
  wagon_types (STRING_AGG), cargo_names (STRING_AGG), container_numbers (STRING_AGG)
```
**Условия отбора:** WHERE is_relevant=true AND id NOT IN (SELECT DISTINCT tw.waybill_id FROM trip_waybills tw JOIN wagon_trips wt ON wt.id = tw.wagon_trip_id WHERE wt.number_train = :train_number AND wt.is_active = true)
**Назначение:** Для будущего использования при необходимости (например, nested tabs в TrainComposition).

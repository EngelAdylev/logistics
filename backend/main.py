        ...
        with engine.connect() as conn:
            ...
            for col, sql in [
                ...
                ("railway_route_number_seq",
                 "CREATE SEQUENCE IF NOT EXISTS railway_route_number_seq START 1"),
                ("railway_routes", """
                    CREATE TABLE IF NOT EXISTS railway_routes (
                        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                        train_number TEXT NOT NULL UNIQUE,
                        route_number INTEGER NOT NULL DEFAULT nextval('railway_route_number_seq'),
                        train_index TEXT,
                        snapshot_data JSONB,
                        route_payload JSONB,
                        route_payload_created_at TIMESTAMPTZ,
                        status TEXT DEFAULT 'open',
                        created_at TIMESTAMPTZ DEFAULT now(),
                        updated_at TIMESTAMPTZ DEFAULT now()
                    )
                """),
                ("railway_routes_add_route_number",
                ...
                ("railway_routes_add_route_payload",
                 "ALTER TABLE railway_routes ADD COLUMN IF NOT EXISTS route_payload JSONB"),
                ("railway_routes_add_route_payload_created_at",
                 "ALTER TABLE railway_routes ADD COLUMN IF NOT EXISTS route_payload_created_at TIMESTAMPTZ"),
                ...
                """),
                # Пересчитать is_relevant для всех накладных на основе статуса
                ("fix_etran_is_relevant", """
                    UPDATE etran_waybills SET is_relevant = false

...
def sync_dislocation_to_tracking():
    ...
    try:
        ...
        # Условие 1: (rem<=0 И код='20') ИЛИ код='96' → архив
        ...
        restored = db.execute(text("""
            ...
            WHERE tw.is_active = false
              ...
              AND NOT EXISTS (
                  ...
                  WHERE le3.rn = 1 AND le3.op_code = '96'
        ...
        for row in results:
            try:
                ...
                try:
                    ...
                    _rem_val = 0
                is_unloaded = op_code == "96" or (_rem_val <= 0 and op_code in ("20",))
